...
\textbf{Observaciones:} Gecode: 100\% triviales. Chuffed: 27\% cr\'iticos. HiGHS: 0\% problem\'aticos, 45\% perfectos. \textbf{HiGHS claramente superior.}
...
\textbf{Observaciones:}
\begin{itemize}
    \item \textbf{Est.1 vs Est.2:} \texttt{dom\_w\_deg} reduce makespan 55\% (134.6 $\rightarrow$ 61.1) y desbalance 99\% (134.6 $\rightarrow$ 1.5)
    \item \textbf{Est.1 vs Est.3:} Calidad id\'entica, pero Est.3 usa 42\% m\'as nodos y 22\% m\'as propagaciones
    \item \textbf{Est.2 eficiencia:} Mejor calidad con menos nodos promedio que Est.1 (5.4M vs 8.4M), pero alta varianza
    \item \textbf{data03 notable:} Est.2 resuelve con solo 4K nodos (balanceo perfecto), vs 9M en Est.1
\end{itemize}
...
\textbf{Conclusi\'on:} HiGHS sin anotaciones supera a Gecode con \texttt{dom\_w\_deg}, demostrando que heur\'isticas internas bien dise\~nadas eliminan necesidad de anotaciones expl\'icitas para las instacias probadas en este problema.
...
\begin{itemize}
    \item \textbf{Est.1:} Makespan prom. 134.6, desbalance 134.6 (100\% triviales), 8.4M nodos
    \item \textbf{Est.2:} Makespan prom. 61.1 (55\% mejor), desbalance 1.5 (99\% mejor), 5.4M nodos
    \item \textbf{Est.3:} Makespan prom. 134.6 (id\'entico a Est.1), 11.9M nodos (42\% peor eficiencia)
    \item \textbf{Est.2 eficiencia variable:} data03 con 4K nodos vs data00 con 11M nodos (alta dependencia de estructura)
\end{itemize}

\subsection{Conclusiones}

\begin{enumerate}
    \item \textbf{HiGHS es el mejor solver para este problema:} Sin anotaciones de b\'usqueda, logra makespan promedio 56.7 (58\% mejor que Gecode sin anotaciones, 7\% mejor que Gecode con \texttt{dom\_w\_deg}), desbalance m\'aximo 14, y solo ~2.5K nodos. Sus heur\'isticas internas son superiores.
    
    \item \textbf{Las heur\'isticas por defecto var\'ian dr\'asticamente:} Gecode produce soluciones triviales (100\% casos), Chuffed tiene 27\% casos cr\'iticos, HiGHS 0\% casos problem\'aticos. La elecci\'on del solver es tan importante como la estrategia de b\'usqueda.
    
    \item \textbf{Anotaciones expl\'icitas necesarias en Gecode:} \texttt{dom\_w\_deg} mejora Gecode 55\% (makespan 134.6 $\rightarrow$ 61.1), pero a\'un queda 8\% por debajo de HiGHS sin anotaciones. Las heur\'isticas adaptativas son efectivas pero no compensan totalmente deficiencias del solver base.
    
    \item \textbf{Decidir operarios primero es contraproducente:} Estrategia 3 produce resultados id\'enticos a Estrategia 1 pero con 42\% m\'as nodos. El orden temporal debe decidirse antes que asignaciones de recursos.
